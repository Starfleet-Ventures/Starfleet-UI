  ...
  try{
    ...
    fs.readdirSync(outputImgPath).forEach(file => {
      if(file.includes(req.query.image)){
        files.push(`http://ec2-13-127-219-224.ap-south-1.compute.amazonaws.com/map/output/${user}/${file}`)
      }
    });
...
router.post('/detect',async (req,res)=>{
    ...
    flag === true?res.status(200).json({processedImageUrl: `http://ec2-13-127-219-224.ap-south-1.compute.amazonaws.com/map/output/${user}/${modelType}_${imageName}`}):res.status(500).json({error: 'Work In Progress'});
})
